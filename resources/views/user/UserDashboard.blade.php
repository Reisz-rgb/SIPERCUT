...
@php
    ...
    // ====== LOGIKA FINAL (SAMA KONSEP: 12 - TERPAKAI TAHUNAN) ======
    // Kuota tahunan default 12
    $quotaTahunan = (int) ($quotaTahunanMax ?? 12);

    // TERPAKAI tahunan (hari cuti tahunan APPROVED) -> dari controller (sebelumnya totalQuota)
    $tahunanUsedDays = (int) ($totalQuota ?? 0); // misal 5

    // SISA tahunan (angka besar biru) = 12 - 5 = 7
    $sisaTahunan = max(0, $quotaTahunan - $tahunanUsedDays);

    // Bar biru = persentase sisa tahunan
    $pctTahunan = ($quotaTahunan > 0) ? ($sisaTahunan / $quotaTahunan) * 100 : 0;
    $pctTahunan = min(100, max(0, $pctTahunan));

    // ORANGE: jumlah pengajuan APPROVED (tahun ini) -> dari controller usedLeave
    $used = (int) ($usedLeave ?? 0);
    $pctUsed = (float) ($percentOrange ?? 0);

    // HIJAU: sisa kuota total (24 - jumlah approved)
    $totalAwal = (int) ($totalAwal ?? 24);
    $sisaTotal = (int) ($remainingLeave ?? max(0, $totalAwal - $used));
@endphp
...
<section class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start mt-6">
    <div class="lg:col-span-5 space-y-4">
        ...
        {{-- ✅ BIRU: Sisa Jatah Tahunan (12 - terpakai tahunan) --}}
        <div class="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm hover:-translate-y-1 transition-transform flex items-center gap-5">
            ...
            <div class="flex-1">
                ...
                <div class="flex items-baseline gap-2">
                    {{-- ANGKA BESAR BIRU = SISA (7) --}}
                    <h3 class="text-3xl font-black text-slate-800 leading-none">{{ $sisaTahunan }}</h3>
                    <p class="text-[11px] text-slate-400 font-bold uppercase">Hari</p>
                </div>

                <div class="mt-2 text-[11px] text-slate-400 font-semibold">
                    Kuota: {{ $quotaTahunan }} • Terpakai: {{ $tahunanUsedDays }}
                </div>

                {{-- BAR BIRU DINAMIS (ngikut sisa) --}}
                <div class="mt-3 bg-slate-100 rounded-full h-1.5 overflow-hidden">
                    <div class="bg-blue-500 h-full" style="width: {{ $pctTahunan }}%"></div>
                </div>
            </div>
        </div>

        {{-- ✅ ORANGE: Telah Diambil (jumlah pengajuan approved) --}}
        <div class="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm hover:-translate-y-1 transition-transform flex items-center gap-5">
            ...
            <div class="flex-1">
                ...
                <div class="flex items-baseline gap-2">
                    <h3 class="text-3xl font-black text-orange-600 leading-none">{{ $used }}</h3>
                    <p class="text-[11px] text-slate-400 font-bold uppercase">Kali</p>
                </div>
                <div class="mt-3 bg-slate-100 rounded-full h-1.5 overflow-hidden">
                    <div class="bg-orange-500 h-full" style="width: {{ $pctUsed }}%"></div>
                </div>
            </div>
        </div>

        {{-- ✅ HIJAU: Sisa Cuti Tersedia --}}
        <div class="bg-white rounded-3xl p-6 border border-emerald-100 shadow-sm hover:-translate-y-1 transition-transform flex items-center gap-5 bg-gradient-to-r from-white to-emerald-50/30">
            ...
            <div class="flex-1">
                ...
                <div class="flex items-baseline gap-2">
                    <h3 class="text-4xl font-black text-slate-800 leading-none">{{ $sisaTotal }}</h3>
                    <p class="text-[11px] text-slate-400 font-bold uppercase">Kali</p>
                </div>
                <div class="mt-2 text-[11px] text-slate-400 font-semibold">
                    Total awal: {{ $totalAwal }} • Terpakai: {{ $used }}
                </div>
            </div>
        </div>
    </div>
                    ...
                    <div class="flex items-center gap-4 p-4 rounded-2xl border border-slate-50 hover:bg-slate-50 transition-all group">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-slate-500 font-medium mt-1">
                                ...
                                <span class="text-slate-800">{{ (int)$leave->duration }} Hari</span>
                            </p>
                        </div>
                    </div>
                @empty
