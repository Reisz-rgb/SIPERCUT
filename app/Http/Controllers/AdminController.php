...
class AdminController extends Controller
{
    ...
    public function updateStatus($id, Request $request)
    {
        ...
        $pengajuan->rejection_reason = $validated['rejection_reason'] ?? null;

        $pengajuan->save();

        // Sync saldo cuti jika pengajuan tahunan di-approve / diubah statusnya
        if ($pengajuan->jenis_cuti === 'Cuti Tahunan') {
            $year = \Carbon\Carbon::parse($pengajuan->start_date)->year;
            LeaveBalance::recalculateAnnualBalances((int)$pengajuan->user_id, (int)$year);
        }
